...
def train_agent(num_games, starting_chips):
    ...
    current_starting_player = "P1"  # Initialize with agent as Player 1

    for _ in range(num_games):
        # Alternate starting player
        if current_starting_player == "P1":
            winner, player_card, opponent_card, actions, player_chips, opponent_chips = play_game(
                agent_model, player_chips, opponent_chips
            )
            current_starting_player = "P2"  # Switch for next round
        else:
            # Reverse roles for the round
            winner, opponent_card, player_card, actions, opponent_chips, player_chips = play_game(
                agent_model, opponent_chips, player_chips
            )
            # Reverse the actions for proper logging
            actions = [(("P2" if action[0] == "P1" else "P1"), action[1]) for action in actions]
            current_starting_player = "P1"  # Switch for next round
