...
class Kunh:
    ...
    def cfr(self, history):
        ...
        if player == self.current_player:
            ...
            for act in range(self.n_actions):
                # print(node.action_dict, '\n')
                next_history = history + node.action_dict[act]
                action_utils[act] = -1 * self.cfr(next_history)
                # print(action_utils)
                
            util = np.sum(action_utils * strategy)
            # print(player_card,history," strat: ", strategy)
            # print(action_utils * strategy)
            regrets = action_utils - util
            # print(regrets," = ", action_utils, " - ", util)
    ...
    def get_reward(history, player_card, opponent_card):
        ...
        # print("P1: ",player_card, ", P2: ", opponent_card, "actions: ", history)
        if terminal_pass:
            if history[-2:] == 'pp':
...
if __name__ == "__main__":
    time1 = time.time()
    trainer = Kunh()
    trainer.train(n_iterations=20000)
